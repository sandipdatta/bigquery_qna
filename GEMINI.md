...
**Note:** All Gemini models mentioned in this document should use `gemini-2.5` variants (e.g., `gemini-2.5-pro`, `gemini-2.5-flash`) as per user preference.
...
4.  **Model and Tool Callbacks (LLM as a Guardrail)**: Use callbacks to inspect inputs/outputs.
    ...
    *   **LLM-based Safety**: Use a cheap/fast LLM (e.g., Gemini Flash) in a callback to classify input/output safety.
        ...
        def safety_checker_callback(context: CallbackContext, llm_request: LlmRequest) -> Optional[LlmResponse]:
            ...
            safety_llm_agent = Agent(name="SafetyChecker", model="gemini-2.5-flash", instruction="Classify input as 'safe' or 'unsafe'. Output ONLY the word.")
...
When building agents that interact with BigQuery, it's important to remember that many large datasets use partitioning to improve performance and manage costs. This means that queries against these tables will often require a `WHERE` clause that filters on the partitioning column.

When designing your agent, make sure to:

1.  **Fetch table metadata:** In addition to the table schema, retrieve the table's partitioning and clustering information.
2.  **Update agent instructions:** Explicitly instruct the agent to check for partitioning information and to include the necessary `WHERE` clause in its generated SQL queries.
3.  **Provide a default date:** If the user doesn't specify a date for a partitioned table, the agent should be instructed to use a recent default date (e.g., `2023-12-31`).
---
